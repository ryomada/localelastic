...
public class SimpleSmartChineseAnalysisTests {
    ...
    public void testDefaultsIcuAnalysis() {
        ...
        Injector injector = new ModulesBuilder().add(
                ...
                new AnalysisModule(EMPTY_SETTINGS, parentInjector.getInstance(IndicesAnalysisService.class)).addProcessor(new KrLuceneAnalysisBinderProcessor()))
                .createChildInjector(parentInjector);

        AnalysisService analysisService = injector.getInstance(AnalysisService.class);

        TokenizerFactory tokenizerFactory = analysisService.tokenizer("smartcn_sentence");
        MatcherAssert.assertThat(tokenizerFactory, instanceOf(KrLuceneSentenceTokenizerFactory.class));

        TokenFilterFactory filterFactory = analysisService.tokenFilter("smartcn_word");
        MatcherAssert.assertThat(filterFactory, instanceOf(KrLuceneWordTokenFilterFactory.class));
    }
}
